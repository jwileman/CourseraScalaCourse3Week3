...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    means.map(point => (point, IndexedSeq())).toMap ++ points.groupBy(point => findClosest(point, means))
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    ...
    //for (i <- (0 until oldMeans.length)) yield findAverage(oldMeans(i), classified(oldMeans(i)))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (for (i <- (0 until oldMeans.length)) yield oldMeans(i) squareDistance newMeans(i)).forall(_ <= eta)
  }
